...
double CollisionCost(const vector<vector<double>>& trajectory, const vector<vector<vector<double>>>& relevant_vehicles,
                        const vector<double>& map_waypoints_x, const vector<double>& map_waypoints_y, double& minimum_distance)
{
    ...
    for  (int i=0; i<relevant_vehicles[lane].size(); i++)
    {
        ...
        double vx = relevant_vehicles[lane][i][3];
        double vy = relevant_vehicles[lane][i][4];
